...
counterfactual_model <- lm(
  log_v ~ log_u,
  data = df_model |>
    filter(post_2020 == FALSE)
)
...
ggplot(df_model,
  ...
  labs(
    title = "Havaittu ja kontrafaktuaalinen työpaikkojen määrä",
...
Jos matching efficiency heikkenee, työmarkkinoilla on enemmän täyttämättömiä paikkoja samalla työttömyystasolla.

Tämä voidaan tulkita usealla tavalla:

osaamisjakauma on muuttunut
työpaikkojen vaatimukset ovat muuttuneet
rekrytointiprosessit ovat muuttuneet
maantieteellinen liikkuvuus on vähentynyt
