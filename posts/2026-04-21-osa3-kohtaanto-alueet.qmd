...
```{r}
#| label: data-haku
#| code-summary: "Datan haku Tilastokeskuksen rajapinnasta (cache-tuella)"

# Tilastokeskus on päivittänyt PxWeb-rajapinnan muuttujien nimet.
# Vanhat nimet ("Kuukausi", "Tiedot") on korvattu standardoiduilla koodeilla
# ("timeperiod_m", "contentscode").
# Myös URL on lyhentynyt: "statfin_tyonv_pxt_12ti.px" → "12ti.px"

# Paikallinen cache: jos data on jo levyllä, luetaan se sieltä.
# Muuten haetaan rajapinnasta ja tallennetaan. Tämä tekee GitHub Actions
# -ajoista vakaampia (Tilastokeskus saattaa estää API-pyynnöt CI-ympäristöistä)
# ja nopeampia.

data_path <- 
  here(
  "data", "raw", "unemployment", "maakunnat_kohtaanto.rds")

if (file.exists(data_path)) {
  raw <- readRDS(data_path)
  cat("Luettu cache-tiedostosta:", data_path, "\n")
} else {
  url <- "https://pxdata.stat.fi/PxWeb/api/v1/fi/StatFin/tyonv/12ti.px"
  ...
  # Luodaan kansio jos sitä ei ole, ja tallennetaan
  dir.create(dirname(data_path), recursive = TRUE, showWarnings = FALSE)
  saveRDS(raw, data_path)
  cat("Haettu rajapinnasta ja tallennettu:", data_path, "\n")
}

# Tarkistus
cat("Rivejä:", nrow(raw), "\n")
cat("Sarakkeet:", paste(names(raw), collapse = ", "), "\n")
```
...
# Koska PxWeb-rajapinnan sarakerakenne voi muuttua API-päivitysten yhteydessä,
# tunnistetaan sarakkeet sisällön perusteella sen sijaan että nojattaisiin
# tarkkoihin nimiin.

# Etsitään aikasarake (sisältää muotoa "2008M01"), työttömien ja avointen
# sarakkeet
aika_sar <- names(raw)[map_lgl(raw, ~ is.character(.x) &&
                                       any(str_detect(.x, "^\\d{4}M\\d{2}$"), na.rm = TRUE))][1]

if (is.na(aika_sar)) {
  # Fallback: kokeillaan tunnettuja nimiä
  if ("kuukausi" %in% names(raw))      aika_sar <- "kuukausi"
  else if ("timeperiod_m" %in% names(raw)) aika_sar <- "timeperiod_m"
}

tyottomat_sar <- names(raw)[str_detect(names(raw),
                                       "tyot|ttom|tiedot_arvo")][1]
avoimet_sar   <- names(raw)[str_detect(names(raw), "avoi")][1]

cat("Tunnistetut sarakkeet:\n")
cat("  aika      :", aika_sar, "\n")
cat("  tyottomat :", tyottomat_sar, "\n")
cat("  avoimet   :", avoimet_sar, "\n")

df <- raw |>
  rename(
    aika_raw  = all_of(aika_sar),
    tyottomat = all_of(tyottomat_sar),
    avoimet   = all_of(avoimet_sar)
  ) |>
  mutate(
    vuosi     = as.integer(str_sub(aika_raw, 1, 4)),
    kk        = as.integer(str_sub(aika_raw, 6, 7)),
...
*Huom:* jos `rename`-rivissä tulee virhe ("column not found"), sarakenimet ovat voineet muuttua API-päivityksen jälkeen. Tarkista silloin `names(raw)` ja päivitä nimet vastaavasti. Tavallisimmat variantit ovat `tiedot_arvo` / `avoimet_tyopaikat_kuukauden_laskentapaivana_lkm`.
...
Tyypillisesti kuva näyttää seuraavaa:

- Uusimaa ja Ahvenanmaa ovat yleensä jakauman yläpäässä
- Kainuu, Pohjois-Karjala, Lappi ja Etelä-Savo ovat alapäässä
- Pohjois-Savo asettuu tyypillisesti keskitason tuntumaan
...
valitut <- c("Uusimaa", "Pirkanmaa", "Pohjois-Savo", "Pohjois-Karjala")
...
Paikallisen näkökulman vuoksi tarkastellaan vielä Pohjois-Savoa erikseen. Siilinjärvi sijaitsee siellä, samoin Kuopio ja Iisalmi.
