...
#   "Postinumeroalueittainen_avoin_tieto_2023/",
#   "paavo_pxt_12f7.px"
# )
# 
...
#  Muunna data frameksi — column.name.type = "code" antaa muuttujanimet koodina
df_uusimaa <- as.data.frame(paavo_raw, column.name.type = "code", variable.value.type = "code") |>
  ...
  mutate(
    ...
    pinta_ala_km2 = if_else(
      sum(pinta_ala, na.rm = TRUE) > 1e5,
      pinta_ala / 1e6,   # m² -> km²
      pinta_ala          # jo km²
    )
  )
...
ihmiset_sateessa   <- tiheys_km2 * osuma_ala_km2 * rakennettu_osuus
...
p_kuolema_kokonais <- p_geometrinen * ihmiset_sateessa * p_kuolema_per_henkilo
...
sim <- tibble(
  ...
  mutate(
    ...
    p_kuolema  = p_geo_sim * ihmiset * tappavuus_sim
  )
...
### BYM2-malli: spatiaalisesti silotellut riskialueet

Käytetään Besag-York-Mollié (BYM2) -mallia INLA:lla. BYM2 on standardi bayesilainen hierarkkinen malli aluedatalle — se lainaa tilastollista voimaa naapurialueilta ICAR-priori-rakenteen kautta, jolloin yksittäisen postinumeroalueen epävarma estimaatti siloittuu naapureidensa avulla.

Tämä on kuitenkin laskennallisesti raskasta ja todennäköisyys on jo hyvin lähellä nollaa, joten tähän ei ole tällä aikataululla järkevää lähteä.
---
