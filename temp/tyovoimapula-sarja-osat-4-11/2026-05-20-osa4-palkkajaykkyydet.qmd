...
# Tilastokeskus: Ansiotasoindeksi (ATI), neljännesvuosittain
# Taulukko: palkat / 11yo
url_ati <- "https://pxdata.stat.fi/PxWeb/api/v1/fi/StatFin/ati/statfin_ati_pxt_11yo.px"

# Oletus: haetaan sopivin indeksi; mukauta tarvittaessa
query_ati <- pxweb_query(list(
  "Tiedot"      = c("*"),
  "Vuosineljännes" = c("*")
))

# Yritetään hakea, jos epäonnistuu niin käytetään Eurostat-pohjaista sarjaa
ati_data <- tryCatch(
  {
    raw <- pxweb_get_data(url = url_ati, query = query_ati, column.name.type = "text") |>
      as_tibble()
    raw
  },
...
# Varmuudeksi Eurostat: labour cost index
...
# Eurostat: LCI-sarjat vertailumaille
